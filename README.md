...
1. **取込**: A/B 2本の音声ファイル（MP3 / WAV / M4A / AAC / FLAC / OGG / Opus / AIFF）をスロットに入れ、**保存先（作業フォルダ）を選んで** `取り込みを開始`。保存先を選ぶまで開始ボタンは押せません。無音検出（VAD）→ 波形生成が走り、進捗はオーバーレイとトップバーに表示されます。 無音判定は「詳細設定（無音判定）」で調整できます — **語尾が削られる場合は「語尾の余白」を増やしてください**（既定 200ms）。
