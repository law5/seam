...
編集の合間にディスクを空けたいときは、閉じるダイアログの**「アーカイブして閉じる」**が使えます。中間WAV（`speakerX_normalized.wav`。90分素材で約518MB/トラック）だけを削除し、次にそのプロジェクトを開くとき元の音源から自動で復元します（60分素材で数分。VAD・文字起こし・波形ピークは再実行されません）。復元には元の音源ファイルが必要なので、元音源が作業フォルダに無いプロジェクトはアーカイブできません。復元した音声はアーカイブ時に記録したサンプル数と照合され、一致しない場合（ffmpeg のバージョン差など）はエラーとして知らせます。
...
1. **取込**: A/B 2本の音声ファイル（MP3 / WAV / M4A / AAC / FLAC / OGG / Opus / AIFF）をスロットに入れ、**保存先（作業フォルダ）を選んで** `取り込みを開始`。保存先を選ぶまで開始ボタンは押せません。無音検出（VAD）→ 波形生成が走り、進捗はオーバーレイとトップバーに表示されます。 無音判定は「詳細設定（無音判定）」で調整できます — **語尾が削られる場合は「語尾の余白」を増やしてください**（既定 200ms）。 感度を一番緩くしても小声・囁きが切られる場合は、「無音とみなす音量」の自動を外してしきい値（目安 -50dBFS）を指定してください。
